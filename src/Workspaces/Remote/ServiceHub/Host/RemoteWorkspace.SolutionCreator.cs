...
#pragma warning restore
            ...
            private static int s_addedCount = 0;
            private static int s_removeRefCount = 0;
            private static int s_removedCount = 0;
            private static int s_changedCount = 0;
                ...
                CancellationToken cancellationToken)
            {
                ...
                foreach (var (projectId, newProjectChecksums) in newProjectIdToStateChecksums)
                {
                    if (!oldProjectIdToStateChecksums.ContainsKey(projectId))
                    {
                        ...
                        Interlocked.Increment(ref s_addedCount);
                ...
                foreach (var (projectId, newProjectChecksums) in newProjectIdToStateChecksums)
                {
                    ...
                    {
                        Interlocked.Increment(ref s_removeRefCount);
                ...
                foreach (var (projectId, _) in oldProjectIdToStateChecksums)
                {
                    ...
                    {
                        Interlocked.Increment(ref s_removedCount);
            ...
            private static int s_UpdateProjectsAsync_ChangedInfoCount = 0;
            private static int s_UpdateProjectsAsync_ChangedCompilationOptionsCount = 0;
            private static int s_UpdateProjectsAsync_ChangedParseOptionsCount = 0;
            private static int s_UpdateProjectsAsync_ChangedProjectReferencesCount = 0;
            private static int s_UpdateProjectsAsync_ChangedMetadataReferencesCount = 0;
            private static int s_UpdateProjectsAsync_ChangedAnalyzerReferencesCount = 0;
            private static int s_UpdateProjectsAsync_ChangedDocumentsCount = 0;
            private static int s_UpdateProjectsAsync_ChangedAdditionalDocumentsCount = 0;
            private static int s_UpdateProjectsAsync_ChangedAnalyzerConfigDocumentsCount = 0;
            ...
            private async Task<Solution> UpdateProjectsAsync(Solution solution, ArrayBuilder<(ProjectId ProjectId, ProjectStateChecksums OldProjectChecksums, ProjectStateChecksums NewProjectChecksums)> projectInfos, CancellationToken cancellationToken)
            {
                ...
                foreach (var projectInfo in projectInfos)
                {
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedInfoCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedCompilationOptionsCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedParseOptionsCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedProjectReferencesCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedMetadataReferencesCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedAnalyzerReferencesCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedDocumentsCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedAdditionalDocumentsCount);
                    ...
                    {
                        Interlocked.Increment(ref s_UpdateProjectsAsync_ChangedAnalyzerConfigDocumentsCount);
                ...
                CancellationToken cancellationToken)
            {
                ...
                // changed documents
                using var _ = ArrayBuilder<(DocumentId documentId, DocumentStateChecksums oldDocumentChecksums, DocumentStateChecksums newDocumentChecksums)>.GetInstance(out var changedDocumentIdsBuilder);
                foreach (var (documentId, newDocumentChecksums) in newDocumentIdToStateChecksums)
                {
                    ...
                    changedDocumentIdsBuilder.Add((documentId, oldDocumentChecksums, newDocumentChecksums));
                }

                if (changedDocumentIdsBuilder.Count > 0)
                    solution = await UpdateDocumentsAsync(solution, changedDocumentIdsBuilder, cancellationToken).ConfigureAwait(false);
            ...
            private async Task<Solution> UpdateDocumentsAsync(
                Solution solution,
                ArrayBuilder<(DocumentId documentId, DocumentStateChecksums oldDocumentChecksums, DocumentStateChecksums newDocumentChecksums)> changedDocumentIdsBuilder,
                CancellationToken cancellationToken)
            {
                using var _1 = ArrayBuilder<(DocumentId documentId, SourceText sourceText)>.GetInstance(out var documentInfoBuilder);
                using var _2 = ArrayBuilder<(DocumentId documentId, SourceText sourceText)>.GetInstance(out var additionalDocumentInfoBuilder);
                using var _3 = ArrayBuilder<(DocumentId documentId, SourceText sourceText)>.GetInstance(out var analyzerConfigDocumentInfoBuilder);

                foreach (var changedDocumentInfo in changedDocumentIdsBuilder)
                {
                    var documentId = changedDocumentInfo.documentId;
                    var oldDocumentChecksums = changedDocumentInfo.oldDocumentChecksums;
                    var newDocumentChecksums = changedDocumentInfo.newDocumentChecksums;
                    ...
                    if (oldDocumentChecksums.Text != newDocumentChecksums.Text)
                    {
                        ...
                        switch (document.Kind)
                        {
                            case TextDocumentKind.Document:
                                documentInfoBuilder.Add((document.Id, sourceText));
                                break;
                            case TextDocumentKind.AnalyzerConfigDocument:
                                additionalDocumentInfoBuilder.Add((document.Id, sourceText));
                                break;
                            case TextDocumentKind.AdditionalDocument:
                                analyzerConfigDocumentInfoBuilder.Add((document.Id, sourceText));
                                break;
                            default:
                ...
                if (documentInfoBuilder.Count > 0)
                    solution = solution.WithDocumentTexts(documentInfoBuilder.ToImmutableAndFree());

                if (additionalDocumentInfoBuilder.Count > 0)
                    solution = solution.WithAnalyzerConfigDocumentTexts(additionalDocumentInfoBuilder.ToImmutableAndFree());

                if (analyzerConfigDocumentInfoBuilder.Count > 0)
                    solution = solution.WithAdditionalDocumentTexts(analyzerConfigDocumentInfoBuilder.ToImmutableAndFree());
