    ...
    : AbstractAssetProvider
{
        ...
        AssetHint assetHint, HashSet<Checksum> checksums, CancellationToken cancellationToken)
    {
        ...
        {
            var usePool = checksums.Count <= PooledChecksumArraySize;
            var missingChecksums = usePool ? s_checksumPool.Allocate() : new Checksum[checksums.Count];
            ...
            for (int i = 0, n = assets.Length; i < n; i++)
                _assetCache.GetOrAdd(missingChecksumsMemory.Span[i], assets[i]);
