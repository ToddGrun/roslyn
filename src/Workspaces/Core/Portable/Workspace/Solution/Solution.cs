...
#pragma warning restore IDE0060 // Remove unused parameter
    ...
    internal Solution WithDocumentTexts(ImmutableArray<(DocumentId, SourceText)> documentIdsAndTexts)
    {
        foreach (var (documentId, sourceText) in documentIdsAndTexts)
        {
            CheckContainsDocument(documentId);

            if (sourceText == null)
            {
                throw new ArgumentException($"null sourcetext in {nameof(documentIdsAndTexts)}");
            }
        }

        var newCompilationState = _compilationState.WithDocumentTexts(documentIdsAndTexts);
        return newCompilationState == _compilationState ? this : new Solution(newCompilationState);
    }
