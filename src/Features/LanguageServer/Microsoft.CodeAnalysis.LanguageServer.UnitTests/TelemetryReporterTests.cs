...
public sealed class TelemetryReporterTests : AbstractLanguageServerHostTests
{
    ...
    public async Task TestBlockLogging()
    {
        ...
        service.LogBlockEnd(blockId: 0, ImmutableArray<(string, object?)>.Empty, CancellationToken.None);
    }
    ...
    {
        var service = await CreateReporterAsync();
        service.Log(GetEventName(nameof(TestLog)), ImmutableArray<(string, object?)>.Empty);
    }
}
