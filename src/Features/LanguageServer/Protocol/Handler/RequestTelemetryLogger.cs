...
internal sealed class RequestTelemetryLogger : IDisposable, ILspService
{
    ...
    public void UpdateTelemetryData(
        ...
        Result result)
    {
        TelemetryLogging.LogAggregated(FunctionId.LSP_TimeInQueue, $"{_serverTypeName}", queuedDuration.Milliseconds);
        TelemetryLogging.LogAggregated(FunctionId.LSP_RequestDuration, $"{methodName}.{_serverTypeName}", requestDuration.Milliseconds);
