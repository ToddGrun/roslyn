...
class c1
{
    ...
    const byte Z2 = (byte)300;
            ...
            var symbol = compilation.GlobalNamespace.GetTypeMembersAsImmutable("c1").First().GetMembersAsImmutable("Z1").First();
            Assert.False(((FieldSymbol)symbol).HasConstantValue);

            symbol = compilation.GlobalNamespace.GetTypeMembersAsImmutable("c1").First().GetMembersAsImmutable("Z2").First();
            Assert.False(((FieldSymbol)symbol).HasConstantValue);
        }
