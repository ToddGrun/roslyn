...
";
            ...
            var global = comp.GlobalNamespace;
            var d = global.GetTypeMembersAsImmutable("D", 0);
...
@"delegate void MyDel(int n);";
            ...
            var comp = CreateCompilation(text);
            var v = comp.GlobalNamespace.GetTypeMembersAsImmutable("MyDel", 0).Single();
            Assert.NotNull(v);
...
@"namespace NS
{
    internal delegate void D<Q>(Q q);
            ...
            Assert.Equal(1, namespaceNS.GetTypeMembersAsImmutable().Length);

            var d = namespaceNS.GetTypeMembersAsImmutable("D").First();
