...
class Test
{
    ...
    public static void Main()
    {
        Run();
            ...
            var stateMachineClass = (NamedTypeSymbol)testClass.GetMembersAsImmutable().Single(s => s.Name.StartsWith("<Run>", StringComparison.Ordinal));
            IEnumerable<IGrouping<TypeSymbol, FieldSymbol>> spillFieldsByType = stateMachineClass.GetMembersAsImmutable().Where(m => m.Kind == SymbolKind.Field && m.Name.StartsWith("<>7__wrap", StringComparison.Ordinal)).Cast<FieldSymbol>().GroupBy(x => x.Type);
