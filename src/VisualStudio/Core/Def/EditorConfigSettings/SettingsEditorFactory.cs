...
internal sealed class SettingsEditorFactory : IVsEditorFactory, IVsEditorFactory4, IDisposable
{
    ...
    private SettingsEditorFactory(Lazy<IComponentModel> componentModel)
    {
        _componentModel = componentModel;
    }

    public static SettingsEditorFactory GetInstance(Lazy<IComponentModel> componentModel)
    {
                                    ...
                                    out int pgrfCDW)
    {
        ...
        var vsEditorAdaptersFactoryService = _componentModel.Value.GetService<IVsEditorAdaptersFactoryService>();
        var settingsDataProviderFactory = workspace.Services.GetRequiredService<ISettingsAggregator>();
        var controlProvider = _componentModel.Value.GetService<IWpfTableControlProvider>();
        var tableMangerProvider = _componentModel.Value.GetService<ITableManagerProvider>();
